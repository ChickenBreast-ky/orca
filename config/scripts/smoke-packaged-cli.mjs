...
function readAppDirArg(argv) {
  ...
  if (process.platform === 'darwin') {
    return 'dist/mac-arm64/Orca Kyle.app'
  }
...
function getPackagedCliPath(appDir) {
  if (process.platform === 'darwin' || appDir.endsWith('.app')) {
    return join(appDir, 'Contents', 'Resources', 'bin', 'orca-kyle')
  }
  if (process.platform === 'win32') {
    return join(appDir, 'resources', 'bin', 'orca-kyle.exe')
  }
  return join(appDir, 'resources', 'bin', 'orca-kyle')
}
