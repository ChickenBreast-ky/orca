...
describe('OrchestrationDb dispatch assignee index migration', () => {
  ...
  it('migrates a populated v22 database to v23 creating the receipt reservation table', () => {
    tempDir = mkdtempSync(join(tmpdir(), 'orca-v22-to-v23-'))
    const dbPath = join(tempDir, 'orchestration.db')
    db = new OrchestrationDb(dbPath)
    ...
    const task = db.createTask({ spec: 'pre-v23 work', runId: run.id })
    const dispatch = db.createDispatchContext(task.id, 'term_worker')
    db.close()
    db = undefined

    // Simulate a real v22 binary: the v23 table and indexes do not exist yet.
    ...
    // Verify the fixture is genuinely v22: no v23 objects present.
    ...
    // First open: v22→v23 migration runs.
    db = new OrchestrationDb(dbPath)
    const sqlite = sqliteFor(db)
    expect(sqlite.pragma('user_version', { simple: true })).toBe(23)
    ...
    // v23 reservation table + indexes created from scratch.
    expect(
      sqlite
        .prepare(
          "SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'dispatch_receipt_reservations'"
    ...
    // Second open: idempotent — still v23, data intact.
    db.close()
    db = new OrchestrationDb(dbPath)
    expect(sqliteFor(db!).pragma('user_version', { simple: true })).toBe(23)
    expect(db!.getDispatchContextById(dispatch.id)).toBeDefined()
