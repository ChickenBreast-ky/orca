...
describe('parseTccPromptEvent', () => {
  ...
  it('separates the accessing binary from the responsible app', () => {
    ...
    expect(event?.responsibleIdentifier).toBe('com.chickenbreastky.orca-kyle')
...
describe('isOrcaAttributedPrompt', () => {
  it('accepts only the app and detached terminal helper across Orca Kyle build identities', () => {
    for (const id of [
      'com.chickenbreastky.orca-kyle',
      'com.chickenbreastky.orca-kyle.helper',
      'com.chickenbreastky.orca-kyle.dev',
      'com.chickenbreastky.orca-kyle.dev.helper',
      'com.chickenbreastky.orca-kyle.local',
      'com.chickenbreastky.orca-kyle.local.helper'
    ]) {
      expect(
        isOrcaAttributedPrompt({
...
describe('MacosTccPromptWatch', () => {
  ...
  it('reports only Orca-attributed dialogs from a live stream', async () => {
    ...
    expect(onPrompt.mock.calls[0][0]).toMatchObject({
      service: 'kTCCServiceSystemPolicyAppData',
      responsibleIdentifier: 'com.chickenbreastky.orca-kyle'
    })
    watch.stop()
  })
