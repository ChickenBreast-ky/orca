#!/bin/bash
# Why: register the bundled `orca-kyle` CLI on PATH at package-install time.
...
for dir in '/opt/Orca Kyle' /opt/orca-kyle; do
  ...
  shim="$dir/resources/bin/orca-kyle"
  if [ -x "$shim" ]; then
    # Only manage our own symlink; never clobber an unrelated /usr/bin/orca-kyle.
